---
layout: default
title: 关注我
description: 在哪里可以关注功夫熊猫：微信公众号「码上驰骋」、视频号、掘金、知乎、dev.to、Zenn 和 GitHub。
permalink: /follow/
lang: zh
---

# 关注我

长文首发在微信公众号，短视频在视频号，代码都在 GitHub。挑一个你常用的地方就好。

## 微信公众号「码上驰骋」

AI 工程实践的长文都先发在这里，包括 pdlc 系列、省 token 系列和 AI 记忆系列。微信扫码关注：

<img src="/assets/images/follow/wechat-qr.jpg" alt="微信公众号「码上驰骋」二维码" width="220" height="220">

## 视频号「码上驰骋」

「AI 编程实录」系列短视频，几分钟讲清一个做法，配套长文在公众号。在微信里搜「码上驰骋」就能找到。

## 技术社区

- **[掘金](https://juejin.cn/user/367705072079994)**：中文技术文章同步
- **[知乎](https://www.zhihu.com/people/kanfu-panda)**：中文技术文章同步
- **[dev.to](https://dev.to/kanfu-panda)**：英文版文章
- **[Zenn](https://zenn.dev/kanfupanda)**：日文版文章

## 开源项目

- **[pdlc-skills](https://github.com/kanfu-panda/pdlc-skills)**：给 AI 编程工具装上完整的开发流程
- **[aitm](https://github.com/kanfu-panda/aitm)**：macOS 上的 AI 原生终端
- **[GitHub 主页](https://github.com/kanfu-panda)**：全部开源项目
